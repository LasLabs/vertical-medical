...
class MedicalPrescriptionOrder(models.Model):
    ...
    def write(self, vals, ):
        ...
        """
        for rec_id in self:
            if rec_id.stage_id.name == 'Verified':
                # Only allow changes for keys in rec_id._ALLOWED_CHANGE_KEYS
                keys = vals.keys()
                for allowed_key in rec_id._ALLOWED_CHANGE_KEYS:
                    try:
            ...
            # Only allow stage changes from rec_id._ALLOWED_CHANGE_STATES
            if vals.get('stage_id'):
                stage_id = vals.get('stage_id')
                if stage_id not in rec_id._ALLOWED_CHANGE_STATES:
                    raise exceptions.ValidationError(_(
